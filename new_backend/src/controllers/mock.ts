...
const createStandardMock = async (req: Request, res: Response, next: NextFunction) => {
  try {
      ...
      const standardMock = new Mock({...value});
...
const getMocksByDepartment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const depId = req.query.depId?.toString();
    const isStandard  = strToBool(req.query.isStandard?.toString());

    if (!isValidObjectId(depId)) {
      throw Error('Invalid department id sent!');
    }

    const departmentId = new ObjectId(depId)
    
    const aggregationPipeline: any[] = [
      {
        $match: { departmentId, isStandard, adminApproval: true }
      },
      {
        $sort: { examYear: -1 }
      },
    ...
    const groupedMocks = await Mock.aggregate(aggregationPipeline);

    let baseResponse = new BaseResponse();
        baseResponse.success = true
        baseResponse.message = 'Mocks retrieved successfully!'
        baseResponse.data = {
            departmentMocks: groupedMocks,
        }
...
const getMocks = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const mocks = await Mock.find().lean().exec();
...
const getMock = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    limit = isNaN(limit) || limit <= 0 ? 200 : limit;

    const mock = await Mock.findById(id).populate({
      path: "questions",
...
const updateMock = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        const mockToBeUpdated = await Mock.findById(id).lean().exec();
...
const deleteMock = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        const mockToBeDeleted = await Mock.findById(id).lean().exec();
...
const downloadMock = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    if (!isValidObjectId(id)) {
      throw new Error('Invalid mock id!');
    }
    const mock = await Mock.findById(id).populate('questions').populate('departmentId').lean().exec();

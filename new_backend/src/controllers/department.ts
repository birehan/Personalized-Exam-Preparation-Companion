...
const createDepartment = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userInput = { description, name, noOfCourses,generalDepartmentId}

        const { error, value } = departmentValidator(userInput,"post");

        if (error) throw error
        
        const foundGeneralDepartment = await GeneralDepartment.findById(generalDepartmentId)
        if(!foundGeneralDepartment) throw Error("General department Id does not exist")
...
const updateDepartment = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let updateObject = { description, name, noOfCourses, generalDepartmentId}

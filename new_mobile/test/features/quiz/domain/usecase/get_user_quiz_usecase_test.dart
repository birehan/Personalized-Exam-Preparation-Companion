  ...
  late MockQuizRepository mockQuizRepository;
   ...
   const questions = 
    [Question(
        ...
        isForQuiz: false)];
  const quiz = Quiz(id: "id", courseId: "courseId", chapterIds:[ "chapterIds"], userId: "userId", name: "name", questionIds: ["questionIds"], questions: questions, score: 5, isComplete: true);

   test(
    "Should create quize from repository",
    () async {
      when(mockQuizRepository.getQuizByCourseId(courseId: "id", isRefreshed: false))
          .thenAnswer((_) async => const Right([quiz]));

      final result = await usecase.call(const GetUserQuizParams(courseId: "id", isRefreshed: false));

      expect(result, const Right([quiz]));

      verify(mockQuizRepository.getQuizByCourseId(courseId: "id", isRefreshed: false));

...
@GenerateNiceMocks([MockSpec<GetCourseWithAnalysisUsecase>()])
void main() {
  ...
  setUp(() {
    ...
    bloc = CourseWithUserAnalysisBloc(
        getCourseUsecase: mockGetCourseWithAnalysisUsecase);
  });
  ...
  group('GetCourseByIdEvent', () {
    test('should get data from the fetch course user analysis usecase',
        () async {
      // arrange
    ...
    test('should emit [Loading, Error] when getting data fails', () async {
      ...
      final expected = [
        CourseLoadingState(),
        CourseErrorState(failure: ServerFailure(), message: "Server Failure")
      ];
      expectLater(bloc.stream, emitsInOrder(expected));
      // act
      bloc.add(GetCourseByIdEvent(id: tId, isRefreshed: false));
    ...
    test(
      ...
      final expected = [
        CourseLoadingState(),
        CourseErrorState(failure: CacheFailure(), message: "Cache Failure")
      ];
      expectLater(bloc.stream, emitsInOrder(expected));
      // act
      bloc.add(GetCourseByIdEvent(id: tId, isRefreshed: false));

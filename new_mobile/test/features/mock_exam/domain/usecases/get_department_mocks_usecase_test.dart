...
void main() {
  ...
  const departmentMocks = [
    DepartmentMock(id: tId, mockExams: [mockExam])
  ];

  test(
    "Should add mock to user mocks response from repository",
    () async {
      when(mockExamRepository.getDepartmentMocks(
              departmentId: tId, isStandard: true, isRefreshed: false))
          .thenAnswer((_) async => const Right(departmentMocks));

      final result = await usecase.call(const DepartmentMocksParams(
          departmentId: tId, isStandard: true, isRefreshed: false));

      expect(result, const Right(departmentMocks));

      verify(mockExamRepository.getDepartmentMocks(
          departmentId: tId, isStandard: true, isRefreshed: false));
